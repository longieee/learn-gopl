...
func TestPopCountClearRightMost(t *testing.T) {
	...
	for _, tc := range tcs {
		ret := PopCountClearRightMost(tc.number)
		if ret != tc.expects {
			t.Errorf("PopCountClearRightMost Failed. Number: %X, expect counts: %d, get: %d", tc.number, tc.expects, ret)
		}
	}
}
...
func BenchmarkPopCountClearRightMost(b *testing.B) {
	for i := 0; i < b.N; i++ {
		PopCountClearRightMost(0xFFFFFFFFFFFFFFFF)
	}
}
